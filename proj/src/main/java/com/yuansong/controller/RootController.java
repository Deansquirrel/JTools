...
public class RootController {
	...
	public ModelAndView dataMaintain(Map<String, Object> model){
		...
		try {
			...
			rData = new HashMap<String, String>();
			List<CommonDbResource>  commonDbResourceList = resourceService.getCommonDbList();
			for(CommonDbResource resource : commonDbResourceList) {
				rData.put("0|" + resource.getId(), resource.getName());
			}
			List<RdsDbResource> rdsDbResourceList = resourceService.getRdsDbList();
			for(RdsDbResource resource : rdsDbResourceList) {
				rData.put("1|" + resource.getId(), resource.getName() + "[RDS]");
			}
			model.put("dbList", rData);
